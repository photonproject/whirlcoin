...
// Copyright (c) 2014 WhirlCoin developers http://WhirlCoin.org
...
double static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    float block;
...
static const int64 nTargetSpacing = 1 * 45; // whirlcoin every 60 seconds
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Prevent blocks from too far in the future [since WhirlCoin version 2.0 at block 100,000]
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "Germany Wins World cup against Argentina 1-0";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = HashWhirlpool(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = HashWhirlpool(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = HashWhirlpool(vRecv.begin(), vRecv.begin() + nMessageSize);
